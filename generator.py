...
from network import gen_topology, count_degree_freqs, calculate_edge_distances, count_distance_ranges, \
...
def find_groups(topo, types, pos, eps=0.1, avoid_single=True):
    ...
    # Retrieve the indexes of the node list that are of a excluded type
    excluded_indices = [index for index, value in list(enumerate(types)) if value in types_excluded]
    ...
    if avoid_single:
        cluster_labels = merge_individual_clusters(nodes, pos, cluster_dict, cluster_labels, types, excluded_indices)
...
def merge_individual_clusters(nodes, pos, cluster_dict, cluster_labels, types, excluded_indices):
    ...
    for old_cluster in range(len(cluster_one_positions)):
        ...
        # Initially they should be one, but as previous iterations may have happen
        ...
        if types[pos_node] in types_excluded:
